...
function plot_example_axis() {
	var svg_elem = d3.select('#svgaxis');
	var pad = 50;
	...
	var enter_selection = plot_group.selectAll('empty').data(iris_data).enter()
	...
	var rect_selection = plot_group.append('rect')
		.attr('fill', 'None').attr('pointer-events', 'all').attr('width', actual_width).attr('height', actual_height)

	plot_group.append('g').attr('id', 'yaxis').attr('transform', 'translate('+0+','+0+')').call(d3.axisLeft(y_scale));
	plot_group.append('g').attr('id', 'xaxis').attr('transform', 'translate('+0+','+(actual_height)+')').call(d3.axisBottom(x_scale));
	...
	mouse_interaction_axis(plot_group.selectAll('circle'), {'xaxis':d3.select('#xaxis'),'yaxis':d3.select('#yaxis')}, {'x':x_scale,'y':y_scale}, rect_selection);
}
