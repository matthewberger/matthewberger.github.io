d3.json('flare.json')
	.then(function(data)  {
		plot_treemap_color(data);
	})

function plot_treemap_color(flare_data)  {
	// process the tree
	...
	var pad = 100;
	var svg_elem = d3.select('#treemap4');
	...
	var lum_scale = d3.scaleLinear().domain(d3.extent(all_nodes, d => d.depth)).range([20,90]);
	var chrom_scale = d3.scaleLinear().domain(d3.extent(all_nodes, d => d.depth)).range([90,20]);
	...
	svg_elem.selectAll('rect').data(all_nodes, d => d.full_name).enter().append('rect')
		...
		.attr('fill', d => d3.hcl(240,chrom_scale(d.depth),lum_scale(d.depth))).attr('stroke', '#999999').attr('stroke-width', d => 0.7);
